...
DEFUN_DLD (sum1skipnan, args, nargout, "OCT-implementation of SUM1SKIPNAN\n") 
//   OCT implementation of SUM1SKIPNAN - this function is part of the NaN-toolbox. 
//
...
//
//
 	// sum1skipnan: adds all non-NaN values
//
// Input:
// - array to sum
...
//  int j, k1, k2, k3, D1, D2, D3, ix1, ix2, N;
  int j, k1, k2, k3, D2, D3, ix1, ix2, N;
#define D1 1
  dim_vector SZ, SZ2;
  double aux, s, si; 
  double n, s2, s4;
...
*/
  if ( args.length() > 2 ) 
  {
    error("SUM1SKIPNAN.OCT takes at most 2 arguments");
    return retval;
  }
...
/*
	...
	if (DIM < 1) 
*/
		DIM=1;		// in case DIM is still undefined 
	
	SZ2 = SZ; 
	//for (j=ND; j<DIM; SZ2(j++)=1);
/*	if (DIM>ND)
	{    	error("SUM1SKIPNAN.OCT: DIM larger than LENGTH(SIZE(X))");
		return retval;
	}	
*/
	if (IS_COMPLEX)
	{    	warning("SUM1SKIPNAN.OCT: imaginary part ignored.");
	}	
