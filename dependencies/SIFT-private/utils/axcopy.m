...
function axcopy(fig, command,keepTicks,magnifyFont)

if ~exist('keepTicks','var')
    keepTicks = true; end
if ~exist('magnifyFont','var')
    magnifyFont = true; end
    
keepTicks   = fastif(keepTicks,'true','false');
magnifyFont = fastif(magnifyFont,'true','false');

if (exist('fig','var') == 1) && strcmp(fig,'noticks')
   noticks = 1;
   if nargin> 1
     shift
   else
...
if ~(exist('fig','var') ==1) || isempty(fig) || fig == 0 
   fig = gcf;
end
...
offidx=[];
if exist('command','var') ~= 1
    comstr = ['copyaxis('''',' keepTicks ',' magnifyFont ')'];
else
   command_dbl = double(command);
   comstr = double(['copyaxis(''' char(command_dbl) ''',' keepTicks ',' magnifyFont ')']); 
end;
...
if exist('command','var') ~= 1
    set(hndl(a),'ButtonDownFcn',['copyaxis('''',' keepTicks ',' magnifyFont ')']);
else
